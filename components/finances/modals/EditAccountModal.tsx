...
export default function EditAccountModal({ account, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await FinanceStore.updateAccount(account.id, { name, bank, balance: parseFloat(balance) || 0, type })
      onClose()
    } finally {
      setLoading(false)
    }
  ...
  async function handleDelete() {
    ...
    setLoading(true)
    try {
      await FinanceStore.deleteAccount(account.id)
      onClose()
    } finally {
      setLoading(false)
    }
  ...
  return (
    <Modal open={account !== null} onClose={onClose} width={440}>
